...
import dataloaders.coco as coco
...
if resume_epoch != nEpochs:
    ...
    voc_train = coco.CocoSegmentation(split='train2014', transform=composed_transforms_tr)
    voc_val = pascal.VOCSegmentation(split='val', transform=composed_transforms_ts, download=True)

    if use_sbd:
        sbd = sbd.SBDSegmentation(split=['train2014', 'val'], transform=composed_transforms_tr, retname=True)
    ...
    trainloader = DataLoader(db_train, batch_size=p['trainBatch'], shuffle=True, num_workers=32)
    ...
    for epoch in range(resume_epoch, nEpochs):
        ...
        for ii, sample_batched in enumerate(trainloader):
            ...
            if ii % 100 == 0:
                ...
                begin = time.time()
            # if inputs.shape[0] < p['trainBatch'] or True:
            #     continue
            if True:
                continue
